 ...
 */
/**
 * Esta clase dibuja varios de los iconos que se usan a lo largo de todo el
 * LookAndFeel. Esta es la clase que hace el trabajo duro de pintar checkboxes,
 * radios, sliders... Consta de varias inner clases privadas y de funciones que
 * las dan acceso. Las inner clases mas o menos son todas iguales: pintan un
 * icono base leido de un archivo PNG, segun cual sea el estado del objeto
 * (seleccionado, inactivo...) le dan una capa de color y si es necesario
 * vuelven a pintar otro icono. Podria hacerse mas sencillo pintando un unico
 * icono segun el estado del componente, teniendo un PNG por cada estado, pero
 * entonces no se podria usar colores en ellos porque si cambiara el color de
 * seleccion o foco, por ejemplo a verde, todos los objetos de la aplicacion se
 * pintarian seleccionados en verde menos los iconos, que se pintarian con el
 * color con que hubieramos pintado el PNG, dando un aspecto inconsistente. Por
 * eso se pinta un icono trasparente y una capa de color cuando hace falta, para
 * decidir en tiempo real cual es el color apropiado.
 *
