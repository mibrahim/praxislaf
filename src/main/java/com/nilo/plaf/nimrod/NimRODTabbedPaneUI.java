 ...
 */
/**
 * Esta clase implementa los JTabbedPane. Practicamente todo el esfuerzo se
 * centra en pintar la pestaa.
 *
...
public class NimRODTabbedPaneUI extends BasicTabbedPaneUI
{
	...
	private MiML miml;
	/**
	 * En este poligono se guarda la forma de la pestaa. Es muy importante.
	 */
		...
		boolean isSelected)
	{
		...
		switch (tabPlacement)
		{
			...
			case BOTTOM:
				xp = new int[]
				{
					x, x, x + 3, x + w - inclTab - 6, x + w - inclTab - 2, x + w - inclTab, x + w - 3, x
				};
				yp = new int[]
				{
					y, y + h - 3, y + h, y + h, y + h - 1, y + h - 3, y, y
				};
				gradientShadow = new GradientPaint(x, y, NimRODUtils.getBrillo(),
					x, y + h, NimRODUtils.getSombra());
				break;
			...
			default:
				xp = new int[]
				{
					x, x, x + 3, x + w - inclTab - 6, x + w - inclTab - 2, x + w - inclTab, x + w, x
				};
				yp = new int[]
				{
		...
		// Encima, pintamos la pestaa con el color que sea
		g2D.fill(shape);

		// Encima, pintamos la pestaa con el color que le corresponde por profundidad
		if (runCount > 1)
		{
		...
		// Y al final le pintamos un bordecito para definir mejor la pestaa
	...
	/**
	 * Este metodo devuelve un tamao mas grande de lo necesario, haciendoer
	...
	/**
	 * Este metodo devuelve un tamao mas grande de lo necesario, haciendo el
	 * hueco para la decoracion.
	 */
	...
	/**
	 * Este metodo dibuja una seal amarilla en la solapa que tiene el foco
	 */
	...
	public class MiML extends MouseAdapter implements MouseMotionListener
	{
		...
		public void mouseMoved(MouseEvent e)
		{
			...
			// Un boton se puede pintar muchas veces porque es pequeo y gasta poco, pero esto es un panel que puede tener cienes y cienes de controles,
